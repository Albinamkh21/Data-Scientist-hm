...
def convert_data_to_table_format():
    ...
    df2.to_csv(TABLE_FORMAT_FILE,sep = ',')
    logger.info(df2)
...
def stats_of_data():
    ...
    #Информация по полу  и городам друзей
    print('Распределение по городам')
    logger.info(df['sex'].value_counts())
    print()
    print('Распределение по городам')
    logger.info(df['city.title'].value_counts())


    #  Анализ друзей по возрасту
    print('Статистики по возрасту')
    df['bdate'] = df['bdate'].astype('str')
    dfSorted = df[df['bdate'].apply(lambda x: len(x) >= 8)]
    dfSorted['age'] = df.bdate.str[-4:]
    dfSorted['age'] = dfSorted['age'].astype('int')
    tday = datetime.datetime.now()
    dfSorted['age2'] = tday.year - dfSorted['age']

    logger.info(dfSorted['age2'].min())
    logger.info(dfSorted['age2'].max())
    logger.info(dfSorted['age2'].mean())


    #  df.insert(len(df.columns), 'age', df.bdate.str[-4:])
